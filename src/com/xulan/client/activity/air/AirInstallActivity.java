 ...
 * 
 */
public class AirInstallActivity extends BaseActivity implements OnClickListener {
	...
	public void initView() {
		...
		edtTaskName = (TextView) findViewById(R.id.air_edt_taskname);
		edtFlight = (EditText) findViewById(R.id.air_flight_txt);
		edtPackageBarcode = (EditText) findViewById(R.id.air_edt_package_code);
		edtPackageNumber = (EditText) findViewById(R.id.air_edt_package_number);

		edtPackageBarcode.setEnabled(false);
		edtPackageNumber.setEnabled(false);
	...
	public void chooseTask(View v){
		...
		intent.putExtra("type", Constant.SCAN_TYPE_AIR);
	...
	public void onClick(View v) {
		...
		case R.id.bill_code_img:
			// 扫描
			Intent openCameraIntent = new Intent(AirInstallActivity.this, CaptureActivity.class);
	...
	public void addData(View v){
		...
		if (MyApplication.m_link_num == 3) {// 第三个环节直接取任务列表名字
			...
			CommandTools.showToast("航班/航次不能为空");
			return;
		}
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			if (data.getPackNumber().equals(strPackageNumber)) {
				...
				data.setFlight(strShipName);
				data.setScanType(Constant.SCAN_TYPE_AIR);
	...
	protected LoadTextNetTask requestGetShip(String userId, final String taskId, int flag) {
			...
			public void onPostExecute(AsyncNetTask t, NetTaskResult result, Object tag) {
				...
				if (result.m_nResultCode == 0) {
					...
					try {
						...
						Toast.makeText(AirInstallActivity.this, "解析错误", 1).show();
						e.printStackTrace();
					}
				} else {
				}
		...
		CustomProgress.showDialog(AirInstallActivity.this, getResources().getString(R.string.searching), false, null);
	...
	protected LoadTextNetTask requestUpload(final List<ScanData> list, final String taskId) {
		...
		CustomProgress.showDialog(AirInstallActivity.this, getResources().getString(R.string.searching), false, null);
		LoadTextNetTask task = UserService.upload(list, taskId, Constant.SCAN_TYPE_AIR, listener, null);
		return task;
	}
